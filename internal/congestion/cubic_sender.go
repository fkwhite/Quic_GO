...
func (c *cubicSender) maybeIncreaseCwnd(
	...
	defer func() {
		...
		fileName := "maybeIncreaseCwnd.log"
		c.writeLog(fileName)
	}()
	// } rQUIC
	if c.InSlowStart() {
...
func (c *cubicSender) OnRetransmissionTimeout(packetsRetransmitted bool) {
	...
	defer func() {
		...
		fileName := "OnRetransmissionTimeout.log"
		c.writeLog(fileName)
	}()
	// } rQUIC
...
func (c *cubicSender) OnConnectionMigration() {
	...
	defer func() {
		...
		fileName := "OnConnectionMigration.log"
		c.writeLog(fileName)
	}()
	// } rQUIC
	c.hybridSlowStart.Restart()
...
func (c *cubicSender) writeLog(fileName string) {
	timestamp := time.Now().UnixMicro()
	fileName = fmt.Sprint("LOGS_congestionWindow/", fileName)
	f, err := os.OpenFile(fileName, os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		// log.Fatalf("error opening file: %v", err)
		log.Println("WARNING: error opening file: ", err)
	}
	defer f.Close()
	logMessage := fmt.Sprint(timestamp, "	", c.congestionWindow, "\n")
