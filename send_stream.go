...
import (
	...
	"github.com/fkwhite/Quic_GO/internal/flowcontrol"
...
func (s *sendStream) popNewOrRetransmittedStreamFrame(maxBytes protocol.ByteCount) (*wire.StreamFrame, bool /* has more data to send */) {
	...
	if len(s.retransmissionQueue) > 0 {
		...
		// fmt.Println("Datos retransmision: ", f.DataLen()) //CRIS
	...
	if len(s.dataForWriting) == 0 && s.nextFrame == nil {
		// fmt.Println("No hay datos ni next frame") //CRIS
	...
	if sendWindow == 0 {
		// fmt.Println("No hay sendWindow") //CRIS
...
func (s *sendStream) popNewStreamFrame(maxBytes, sendWindow protocol.ByteCount) (*wire.StreamFrame, bool) {
	if s.nextFrame != nil {
		// fmt.Println("Hay next frame") //CRIS
		...
		if nextFrame.DataLen() > maxDataLen {
			// fmt.Println("Se crea otro next frame mas") //CRIS
		...
		// fmt.Println("Datos payload (new frame y + datos): ", nextFrame.DataLen()) //CRIS
		return nextFrame, s.nextFrame != nil || s.dataForWriting != nil
	}
	// fmt.Println("No hay next frame") //CRIS
	...
	if len(f.Data) == 0 && !f.Fin {
		// fmt.Println("Datos payload (new stream) son 0 y fin esta a true") //CRIS
		f.PutBack()
		return nil, hasMoreData
	}
	// fmt.Println("Datos payload (new stream): ", f.DataLen()) //CRIS
	return f, hasMoreData
}
...
func (s *sendStream) TotalQueue() protocol.ByteCount {
	if s.nextFrame != nil {
		return s.RemainingBytes() + s.BytesToRetransmit() + s.nextFrame.DataLen()
	}
	return s.RemainingBytes() + s.BytesToRetransmit()
}

// FramesToRetransmit returns the number of frames enqueued for retransmission
func (s *sendStream) FramesToRetransmit() int {
	return len(s.retransmissionQueue)
}

// BytesToRetransmit returns the number of data bytes excluding frame headers
func (s *sendStream) BytesToRetransmit() protocol.ByteCount {
	var reTxBytes protocol.ByteCount
