...
func (m *streamsMap) HasAppData() bool {
...
func (m *streamsMap) SentBytes() protocol.ByteCount {
	var numBytes protocol.ByteCount
	for _, str := range m.outgoingBidiStreams.streams {
		numBytes += str.SentBytes()
	}
	for _, str := range m.outgoingUniStreams.streams {
		numBytes += str.SentBytes()
	}
...
// RemainingBytes shows current length of the app data whose
func (m *streamsMap) RemainingBytes() protocol.ByteCount {
	var numBytes protocol.ByteCount
	for _, str := range m.outgoingBidiStreams.streams {
		numBytes += str.RemainingBytes()
	}
	for _, str := range m.outgoingUniStreams.streams {
		numBytes += str.RemainingBytes()
	}
...
func (m *streamsMap) TotalQueue() protocol.ByteCount {
	var numBytes protocol.ByteCount
	for _, str := range m.outgoingBidiStreams.streams {
		numBytes += str.TotalQueue()
	}
	for _, str := range m.outgoingUniStreams.streams {
		numBytes += str.TotalQueue()
	}
...
func (m *streamsMap) FramesToRetransmit() int {
	var numFrames int
	for _, str := range m.outgoingBidiStreams.streams {
		numFrames += str.FramesToRetransmit()
	}
	for _, str := range m.outgoingUniStreams.streams {
		numFrames += str.FramesToRetransmit()
	}
...
func (m *streamsMap) BytesToRetransmit() protocol.ByteCount {
	var numBytes protocol.ByteCount
	for _, str := range m.outgoingBidiStreams.streams {
		numBytes += str.BytesToRetransmit()
	}
	for _, str := range m.outgoingUniStreams.streams {
		numBytes += str.BytesToRetransmit()
	}
