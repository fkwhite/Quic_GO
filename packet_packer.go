...
type frameSource interface {
	...
	SchedulerSTBPQueuing([]ackhandler.Frame, protocol.ByteCount) ([]ackhandler.Frame, protocol.ByteCount)
...
func (p *packetPacker) composeNextPacket(maxFrameSize protocol.ByteCount, ackAllowed bool) *payload {
	...
	if hasData {
		...
		switch configuration.Scheduler_name {
		case "RR":
		...
		case "STBP":
			payload.frames, lengthAdded = p.framer.SchedulerSTBPQueuing(payload.frames, maxFrameSize-payload.length)
		case "MaxDelay":
...
func (p *packetPacker) HasAppData() bool {
	return p.framer.HasAppData()
}

// SentBytes returns the number of sent bytes without counting the retransmissions
func (p *packetPacker) SentBytes() protocol.ByteCount {
	return p.framer.SentBytes()
}

// RemainingBytes returns current length of the application data not transmitted yet
func (p *packetPacker) RemainingBytes() protocol.ByteCount {
	return p.framer.RemainingBytes()
}
...
func (p *packetPacker) HasRetransmission() bool {
	return p.framer.HasRetransmission()
}
...
func (p *packetPacker) TotalQueue() protocol.ByteCount {
	return p.framer.TotalQueue()
}

// FramesToRetransmit returns the number of frames enqueued for retransmission
func (p *packetPacker) FramesToRetransmit() int {
	return p.framer.FramesToRetransmit()
}

// BytesToRetransmit returns the number of data bytes excluding frame headers
func (p *packetPacker) BytesToRetransmit() protocol.ByteCount {
	return p.framer.BytesToRetransmit()
}
